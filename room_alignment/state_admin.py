...
def _online_backup(source: Path, destination: Path) -> dict[str, int | str]:
    ...
    try:
        ...
        source_db.close()
    return validate_database(destination)
...
def restore(state_database: Path, backup_database: Path, replace: bool) -> dict[str, int | str]:
    ...
    validate_database(backup_database)
    ...
    try:
        ...
        details = validate_database(state_database)
...
def run_admin(args: argparse.Namespace) -> int:
    ...
    elif args.admin_command == "dry-run-migrate":
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Back up, verify, migration-check, or restore local canonical state"
    )
    add_admin_arguments(parser)
    return run_admin(parser.parse_args(argv))

...
`contracts/openapi.json` is normative for HTTP operations. `contracts/api.schema.json`, `domain.schema.json`, `commands.schema.json`, and `manifest.schema.json` define resource, domain, command, and completed-manifest structures. The service exposes those exact files at `/api/v1/openapi.json` and `/api/v1/contracts/{name}`. `web/api-client.js` is generated from operation IDs and fails verification when stale.

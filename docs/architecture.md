...
One process owns a state directory through a non-blocking OS file lock. Scans probe with bounded worker/queue counts. At most two analysis jobs, one full-hash render-plan creation, and one render are active concurrently; excess work receives stable `JOB_STATE_CONFLICT` backpressure. Event history retains at most 100,000 recent events; derived cache policy is at most 10,000 entries/2 GiB, evicting only registered unpinned files beneath the state cache root.
